...
lista_gitmana = ['Bzowa 1, Gdańsk', 'Obrońców Wybrzeża 10A, Gdańsk']
start = []
end_point=[]
...
if fastest_bike['time'] < fastest_tier['time']:
    ...
    gmaps.get_walk_direction(start[0], (fastest_bike['lat'], fastest_bike['lon']))
    for i in range(len(lista_gitmana)-1):
        if i == 0:
            print(gmaps.get_bike_direction((fastest_bike['lat'], fastest_bike['lon']), lista_gitmana[i+1]))
            continue
        if i == (len(lista_gitmana)-2):
            gmaps.get_bike_direction(lista_gitmana[i], (end_for_mevo['lat'], end_for_mevo['lon']))
            gmaps.get_walk_direction((end_for_mevo['lat'], end_for_mevo['lon']), end_point)
        gmaps.get_bike_direction(lista_gitmana[i], lista_gitmana[i+1])

else:
    print(gmaps.get_walk_direction(start[0], (fastest_tier['lat'], fastest_tier['lon'])))
    for i in range(len(lista_gitmana)-1):
        if i == 0:
            print(gmaps.get_bike_direction((fastest_tier['lat'], fastest_tier['lon']), lista_gitmana[i+1]))
            continue
        print(gmaps.get_bike_direction(lista_gitmana[i], lista_gitmana[i+1]))
